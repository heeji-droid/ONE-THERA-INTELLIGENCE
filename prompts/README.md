...
| `05_AGENDA.md` | **AGENDA** | `coverage.py` 결핍 | 이번 사이클 수집 지시서 |
| `10_COLLECT.md` | RESEARCH | 지시서 + 원자료 | Atomic Insight JSONL |
...
| `60_ASK.md` | **ASK** | 사용자 질문 + 저장소 | 근거 붙은 답 + 신규 인사이트 |

## 실행 예 — 자율 사이클

```bash
# 0. 결핍 확인 — 무엇을 볼지는 기계가 정한다
python3 tools/coverage.py --agenda data/insights/*.jsonl
cat prompts/00_MASTER.md prompts/05_AGENDA.md
...
그리고 05(AGENDA)를 건너뛴 수집은 흥미로운 것만 모으게 되는데,
**흥미로운 것은 대개 이미 아는 것이다.**
