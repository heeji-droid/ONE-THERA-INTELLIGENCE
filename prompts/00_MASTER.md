...
| **N4** | **자료가 말하지 않은 것을 자료가 말한 것처럼 쓰지 않는다.** 추론은 `consumer_behavior` 이후 필드에서만 한다. `observation`은 사실만 |
| **N5** | **통제 어휘를 벗어나지 않는다.** `underlying_need`와 `human_motivation`은 `schema/vocabularies.yaml`에서만 고른다. 필요한 어휘가 없으면 가장 가까운 것을 고르고 `note`에 부족함을 적는다 |
...
✅ `"Tier 4에서 retinol 중단 서사가 12개월간 반복되고(AI-2025-0003, n≈1,000),
   같은 기간 Tier 2는 온라인 검증 단계의 연장을 보고한다(AI-2025-0002).
   두 신호가 가리키는 공통 동기는 loss_aversion이며, 이는 supplements·fitness에서도 동일하게 관찰된다.
   → 약사의 직업적 기술은 '중단'이 아니라 '적정 용량'이다. 이 격차가 기회다."`

## OUTPUT DISCIPLINE

- 각 단계는 **해당 단계의 출력 형식만** 낸다. 앞서가지 않는다.
- 확신의 정도를 항상 표시한다: 사실 / 추론 / 가설
